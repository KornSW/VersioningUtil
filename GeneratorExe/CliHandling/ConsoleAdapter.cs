...
namespace Versioning.CliHandling {
  ...
  internal class ConsoleAdapter : IDisposable {
    ...
    public void RegisterInvokationTarget<TTargetType>(TTargetType instance) {
      RegisterInvokationTarget(typeof(TTargetType), instance);
    }

    public void RegisterInvokationTarget(object instance) {
      RegisterInvokationTarget(instance.GetType(), instance);
    }
    ...
    public void EnterConsoleLoop() {
      EnterConsoleLoop(new CancellationToken());
    }
    ...
    private class CommandInvoker {
      ...
      public int MinParamCount {
        get {
          return MaxParamCount - _OptionalParamCount;
        }
      }
        ...
        if (includeMethodSignature) {
          ...
          if (@params.Any()) {
            foreach (var p in @params) {
              string paraInfoString = $"{FirstToUpper(p.Name)}({p.ParameterType.Name})";
              if (p.IsOptional) {
                string def = "null";
          ...
          try {
            ...
            foreach (var p in @params) {
              ...
              if (!string.IsNullOrWhiteSpace(pDoc)) {
                ...
                //}
                _XmlComment = _XmlComment + $"@{FirstToUpper(p.Name)} -> " + pDoc;
              }
            }
          }
          catch {
      ...
      public string Invoke(string[] args) {
        try {
          if (args.Length > MaxParamCount || args.Length < MinParamCount) {
            if (MaxParamCount == MinParamCount) {
              throw new ArgumentException(string.Format("A Count of {1} Arguments ist expected, but {0} were entered!", args.Length, MaxParamCount));
            } else {
              throw new ArgumentException(string.Format("A Count of {1}-{2} Arguments ist expected, but {0} were entered!", args.Length, MinParamCount, MaxParamCount));
            }
          }
          ...
          Console.WriteLine(GetHelpString(false));
          Console.ForegroundColor = oldColor;
        }
    ...
    public void WriteLine() {
      FoistOutput(string.Empty, true, Console.ForegroundColor);
    }
    public void WriteLine(string line) {
      FoistOutput(line, true, Console.ForegroundColor);
    }
    public void WriteLine(string line, ConsoleColor color) {
      FoistOutput(line, true, color);
    }
    public void Write(string text) {
      FoistOutput(text, false, Console.ForegroundColor);
    }
    public void Write(string text, ConsoleColor color) {
      FoistOutput(text, false, color);
    }
    ...
    private void FoistOutput(string text, bool writeLinebreak, ConsoleColor color) {
      lock (this) {
        ...
        if (_ReadingLine) {
          ...
          MoveBack(_CurrentInputBuffer.Length + 2);
          ...
          if (_LastFoistLineEndPosition > 0) {
            ...
            MoveBack(leftSpaceOnLastFoistLine);
          }
        }
        ...
        lock (this) {
          switch (keyInfo.Key) {
            ...
            case ConsoleKey.Escape: {
              currentAutoCompleteIndex = -1;
              MoveBack(_CurrentInputBuffer.Length + 1);
              _CurrentInputBuffer.Clear();
              break;
            }
            ...
            case ConsoleKey.UpArrow: {
              ...
              if (_LineInputHistory.Count > currentHistoryIndex + 1) {
                MoveBack(_CurrentInputBuffer.Length);
                _CurrentInputBuffer.Clear();
